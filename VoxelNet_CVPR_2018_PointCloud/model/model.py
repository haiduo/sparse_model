...
class RPN3D(nn.Module):
    ...
    def forward(self, data):
        ...
        features = self.feature(vox_feature)
        
        coords = torch.cat(vox_coordinate, dim=0)
        inputs = SparseTensor(coords = coords, feats=features)
        
        prob_output, delta_output = self.rpn(inputs)
        #########
