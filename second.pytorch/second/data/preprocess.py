...
def prep_pointcloud(input_dict,
    ...
    if training:
        ...
        # pc_range = voxel_generator.point_cloud_range #spconv1.2.1
        pc_range = voxel_generator.coors_range #spconv2.1
        group_ids = None
        if "group_ids" in gt_dict:
            group_ids = gt_dict["group_ids"]
        ...
        # bv_range = voxel_generator.point_cloud_range[[0, 1, 3, 4]] #spconv1.2.1
        bv_range = np.array(voxel_generator.coors_range)[[0, 1, 3, 4]] #spconv2.1
    ...
    # [0, -40, -3, 70.4, 40, 1]
    # voxel_size = voxel_generator.voxel_size #spconv1.2.1
    # pc_range = voxel_generator.point_cloud_range #spconv1.2.1
    voxel_size= voxel_generator.vsize, #spconv2.1
    pc_range= voxel_generator.coors_range, #spconv2.1
    grid_size = voxel_generator.grid_size
    # [352, 400]
    t1 = time.time()
    if not multi_gpu:
        # res = voxel_generator.generate(points, max_voxels) #spconv1.2.1
        res = voxel_generator(torch.from_numpy(points).cpu()) ##spconv2.1
        # voxels, coordinates, num_points = res["voxels"], res["coordinates"], res["num_points_per_voxel"] #spconv1.2.1
        voxels, coordinates, num_points = res[0], res[1], res[2] #spconv2.1
